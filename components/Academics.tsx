...
import { TrendingUp, Award, BookOpen, Calculator, Globe, Star, FileCheck, PieChart, BarChart, Info, RefreshCw, ChevronRight, Activity } from 'lucide-react';
...
const Academics: React.FC = () => {
    ...
    const getY = (percent: number) => height - padding - ((percent - 60) * (chartHeight / 40));
    ...
    const renderLine = (type: 'agg' | 'sub', subIndex: number | null, color: string, widthLine: number) => {
        ...
        return (
            <g>
                ...
                {type === 'sub' && (
                    <path d={`${d} L ${getX(examData.length - 1)},${height - padding} L ${getX(0)},${height - padding} Z`} fill="url(#selectedGradient)" className="animate-pulse" />
                )}
                <path d={d} fill="none" stroke={color} strokeWidth={widthLine} strokeLinecap="round" strokeLinejoin="round" className="transition-all duration-700" />
            </g>
        );
    };
    ...
    const renderPoint = (termIdx: number, subIdx: number | 'agg', color: string) => {
        ...
        return (
            <g key={`${termIdx}-${subIdx}`} className="cursor-pointer" onMouseEnter={() => setHoverPoint({ term: termIdx, subIndex: subIdx })} onTouchStart={() => setHoverPoint({ term: termIdx, subIndex: subIdx })} onMouseLeave={() => setHoverPoint(null)}>
            ...
            <div className="relative w-48 h-48 sm:w-56 sm:h-56 md:w-64 md:h-64 flex items-center justify-center mx-auto">
                <svg viewBox="0 0 100 100" className="w-full h-full transform -rotate-90">
                    {data.map((val, i) => {
                        ...
                        const pathData = [`M ${50 + 40 * startX} ${50 + 40 * startY}`, `A 40 40 0 ${largeArcFlag} 1 ${50 + 40 * endX} ${50 + 40 * endY}`].join(' ');
                        return (
                            <path key={i} d={pathData} fill="none" stroke={colors[i]} strokeWidth={selectedSubject === i ? 16 : 10} className={`transition-all duration-300 cursor-pointer hover:stroke-slate-900 ${selectedSubject !== null && selectedSubject !== i ? 'opacity-40' : 'opacity-100'}`} onClick={() => setSelectedSubject(i)} />
                        );
                    })}
                </svg>
                <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4 pointer-events-none">
                    <span className="text-2xl sm:text-3xl font-black text-slate-800 leading-none">
                        {selectedSubject === null ? getAggregatePercentage(activeTerm).toFixed(1) : getSubjectPercentage(activeTerm, selectedSubject).toFixed(1)}%
                    </span>
        ...
        <div className="max-w-6xl mx-auto py-4 md:py-8 space-y-6 md:space-y-12 pb-24 md:pb-32">
            <div className="flex flex-col gap-4 md:gap-8 border-b border-slate-200 pb-6 md:pb-8">
                ...
                <div className="flex flex-wrap gap-1.5 md:gap-2 bg-slate-100 p-1 rounded-xl md:rounded-2xl border border-slate-200 self-start shadow-inner">
                    {examData.map((d, i) => (
                        <button key={i} onClick={() => setActiveTerm(i)} className={`px-3 py-1.5 md:px-5 md:py-2 rounded-lg md:rounded-xl text-[10px] md:text-xs font-bold transition-all ${activeTerm === i ? 'bg-white text-teal-600 shadow-sm border border-slate-200/50' : 'text-slate-500 hover:text-slate-800'}`}>
                            {d.name}
                        </button>
                    ))}
                </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-8">
                ...
                <StatCard icon={Activity} label="Status" value={getAggregatePercentage(activeTerm) > 90 ? 'Exceptional' : 'Outstanding'} color="text-indigo-600" bg="bg-indigo-50" />
                <StatCard icon={Star} label="Best" value={subjects[examData[activeTerm].marks.indexOf(Math.max(...examData[activeTerm].marks))]} color="text-amber-600" bg="bg-amber-50" />
            </div>
            ...
            <div className="grid lg:grid-cols-3 gap-6 md:gap-8">
                <Card className="lg:col-span-2 overflow-hidden flex flex-col p-0">
                    <div className="p-4 md:p-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        ...
                        {selectedSubject !== null && (
                            <button onClick={() => setSelectedSubject(null)} className="flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest text-teal-600 hover:text-teal-700 bg-teal-50 px-3 py-2 rounded-lg border border-teal-100 transition-all active:scale-95">
                                <RefreshCw size={12} /> Reset View
                            </button>
                        )}
                    </div>

                    <div className="overflow-x-auto px-2 md:px-8 pb-6">
                        <table className="w-full text-left min-w-[280px]">
                            <thead>
                                <tr className="text-[9px] md:text-[10px] uppercase tracking-[0.15em] text-slate-400 border-b border-slate-100">
                                    ...
                                    <th className="pb-4 font-black text-center w-16 md:w-24">Marks</th>
                                    <th className="pb-4 font-black hidden sm:table-cell text-center">Efficiency</th>
                                    <th className="pb-4 font-black text-right pr-2">Link</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-50">
                                {subjects.map((sub, idx) => {
                                    ...
                                    return (
                                        <tr key={sub} className={`group cursor-pointer transition-all ${isSelected ? 'bg-slate-50' : 'hover:bg-slate-50/20'}`} onClick={() => setSelectedSubject(idx)}>
                                            <td className="py-3 md:py-5">
                                                <div className="flex items-center gap-2 md:gap-4">
                                                    <div className={`w-7 h-7 md:w-10 md:h-10 rounded-lg md:rounded-xl flex items-center justify-center transition-all ${isSelected ? 'bg-white shadow-md' : 'bg-slate-100 opacity-70'}`} style={{ color: colors[idx] }}>
                                            ...
                                            <td className={`py-3 md:py-5 text-center font-mono font-black text-[11px] md:text-sm whitespace-nowrap ${isSelected ? 'text-teal-600' : 'text-slate-500'}`}>
                                                {score}<span className="text-[9px] opacity-30 ml-0.5 md:ml-1 font-sans">/{maxSub}</span>
                                            </td>
                                            <td className="py-3 md:py-5 px-4 hidden sm:table-cell">
                                                <div className="flex items-center gap-3">
                                                    <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden min-w-[60px]">
                                                        <div className="h-full rounded-full transition-all duration-1000 ease-out" style={{ width: `${percent}%`, backgroundColor: colors[idx] }}></div>
                                                    </div>
                ...
                <Card className="flex flex-col items-center justify-between space-y-6 md:space-y-8 bg-gradient-to-br from-white to-slate-50 border-slate-200">
                    ...
                    <div className="w-full grid grid-cols-1 gap-2">
                        {subjects.map((sub, i) => (
                            <button key={sub} onClick={() => setSelectedSubject(i)} className={`flex items-center justify-between p-2 rounded-lg transition-all border ${selectedSubject === i ? 'bg-white shadow-sm border-slate-200 ring-1 ring-slate-100' : 'hover:bg-slate-100 border-transparent'}`}>
                                <div className="flex items-center gap-3">
                                ...
                                <span className="font-mono text-[10px] md:text-[11px] font-black text-slate-700">{getSubjectPercentage(activeTerm, i).toFixed(1)}%</span>
                            </button>
                        ))}
                    </div>
                </Card>
                ...
                <Card className="relative overflow-hidden group border-slate-200 p-4 md:p-8">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 md:mb-12 relative z-10">
                        <div className="flex items-center gap-3 md:gap-4">
                            ...
                            <div>
                                <h3 className="text-lg md:text-2xl font-black text-slate-800 tracking-tight leading-tight">{selectedSubject === null ? 'Progression Trend' : `${subjects[selectedSubject]} Growth`}</h3>
                    ...
                    <div className="mt-6 md:mt-8 p-6 md:p-8 bg-slate-900 rounded-2xl md:rounded-[2rem] text-white flex flex-col md:flex-row items-center justify-between gap-6 md:gap-10 relative overflow-hidden">
                        ...
                        <div className="relative z-10 space-y-1 md:space-y-2 text-center md:text-left">
                            ...
                            <p className="text-sm md:text-xl font-bold md:font-black leading-snug md:leading-tight max-w-md">
                                {selectedSubject === null ? `Overall consistency is high, with a peak aggregate of ${Math.max(...examData.map((_, i) => getAggregatePercentage(i))).toFixed(1)}%.` : `In ${subjects[selectedSubject]}, your strongest performance reached ${Math.max(...examData.map((_, i) => getSubjectPercentage(i, selectedSubject))).toFixed(1)}%.`}
                            </p>
                        </div>
                        <div className="relative z-10 flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                            <button onClick={() => window.print()} className="bg-white text-slate-900 px-8 py-3.5 rounded-xl md:rounded-2xl font-black uppercase tracking-widest text-[10px] md:text-xs hover:bg-teal-400 hover:text-white transition-all shadow-xl flex items-center justify-center gap-2 active:scale-95">
...
const StatCard = ({ icon: Icon, label, value, color, bg }: any) => (
    <Card className={`group hover:-translate-y-1.5 md:hover:-translate-y-2 transition-all duration-500 flex flex-col justify-between p-3 sm:p-4 md:p-6 border-slate-200 min-h-[110px]`}>
        <div className={`w-8 h-8 md:w-11 md:h-11 rounded-lg md:rounded-xl ${bg} ${color} flex items-center justify-center mb-2 md:mb-5 shadow-sm transition-transform group-hover:rotate-6`}>
            <Icon size={16} className="md:w-6 md:h-6" />
        </div>
        <div className="overflow-hidden">
            <p className="text-[7px] sm:text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-[0.1em] mb-0.5 md:mb-1 leading-none">{label}</p>
            <p className="text-sm sm:text-base md:text-2xl font-black text-slate-800 tracking-tight leading-tight truncate">
                {value}
            </p>
        </div>
    </Card>
